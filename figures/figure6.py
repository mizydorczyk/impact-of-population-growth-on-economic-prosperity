...
def main():
    ...
    gdp_per_capita = gdp_per_capita.dropna(subset=['Country Name'])
    ...
    gdp_per_capita['GDP per capita'] = pd.to_numeric(gdp_per_capita['GDP per capita'])
    gdp_per_capita = gdp_per_capita.dropna(subset=['Year', 'GDP per capita'])
    gdp_per_capita = gdp_per_capita[(gdp_per_capita['Year'] >= 1990) & (gdp_per_capita['Year'] <= 2019)]

    avg_gdp_per_capita = gdp_per_capita.groupby('Country Name')['GDP per capita'].mean().reset_index()
    avg_gdp_per_capita = avg_gdp_per_capita.rename(columns={'GDP per capita': 'Avg GDP per capita'})
    ...
    plt.ylabel('Średnie PKB na osobę')
